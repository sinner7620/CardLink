...
async function uploadArtifact(releaseId) {
  ...
  const attachmentsResp = await fetch(apiUrl(`/releases/${releaseId}/attach_files`), { headers: giteeHeaders() })
  ...
  if (Array.isArray(attachments)) {
    ...
    for (const asset of existingAssets) {
      const deleteResp = await fetch(apiUrl(`/releases/${releaseId}/attach_files/${asset.id}`), { method: "DELETE", headers: giteeHeaders() })
      if (!deleteResp.ok) {
...
const listResp = await fetch(apiUrl("/releases?per_page=100"), { headers: giteeHeaders() })
...
const createResp = await fetch(apiUrl("/releases"), {
  method: "POST",
  headers: { ...giteeHeaders(), "Content-Type": "application/json;charset=UTF-8" },
  body: JSON.stringify({
    tag_name: tag,
