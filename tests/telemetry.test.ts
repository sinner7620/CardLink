...
test("优先使用 xyz 自定义域名，并保留 workers.dev 备用入口", () => {
...
test("上报只接受 204，并在成功后记录时间且失败全程静默", () => {
  ...
  assert.match(source, /TELEMETRY_PRIMARY_ENDPOINT,[\s\S]*TELEMETRY_FALLBACK_ENDPOINT/)
  assert.doesNotMatch(source, /cardlink\.cn\.eu\.org/)
