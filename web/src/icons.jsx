...
const paths = {
  ...
  locate: '<path d="M20 10c0 5-5.5 11.5-7.4 13.5a1 1 0 0 1-1.2 0C9.5 21.5 4 15 4 10a8 8 0 1 1 16 0"/><circle cx="12" cy="10" r="3"/>',
  ...
  eyeOff: '<path d="m4 4 16 16M9.9 5.9A9.8 9.8 0 0 1 12 5.5c6 0 9.5 6.5 9.5 6.5a17.6 17.6 0 0 1-3 3.6M6.1 8.3A16.9 16.9 0 0 0 2.5 12S6 18.5 12 18.5a9.6 9.6 0 0 0 3-.5M9.9 9.9a3 3 0 0 0 4.2 4.2"/>'
}

export function Icon({ name, className = "" }) {
  const path = paths[name] || paths.settings
  if (import.meta.env?.DEV && !paths[name]) {
    console.warn(`[icons] 未知图标名 "${name}"，已回退为 settings`)
  }
  return <svg className={`uiIcon ${className}`} viewBox="0 0 24 24" aria-hidden="true" dangerouslySetInnerHTML={{ __html: path }} />
}
