...
export function answerControlBarLayout(
  ...
  locate: { x: number; y: number; width: number; height: number }
  ...
  const count = candidatesVisible ? 4 : 3
  ...
  return {
    ...
    locate: slot(left ? 1 : count - 2),
    refresh: slot(left ? 2 : count - 3),
    candidates: slot(left ? 3 : 0)
  }
}
