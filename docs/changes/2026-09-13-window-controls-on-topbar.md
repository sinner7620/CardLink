...
用户在体验顶栏直显按钮的多轮调整（直落顶栏、无底色、收紧间距、32px 宽）后要求改回胶囊。将 `web/src/main.jsx`、`web/src/ui/shell.css`、`web/src/a11y.css`、`tests/web-bridge.test.ts` 四个文件整体恢复到胶囊移除前（c440424）的状态：白色胶囊容器（88×36、全圆角、阴影）、windowControlJelly 按压动画、胶囊内触控规则与契约断言全部还原；本会话其他改动（AI 配置页、导出等）不受影响。`pnpm check`、257 项测试、`pnpm build` 通过；浏览器实测胶囊 88×36 已恢复。

## 跟进（同日）：胶囊尺寸对齐顶栏选中导航项

用户反馈 88×36 胶囊过大。实测选中导航按钮高 32px，将 `.windowControlCapsule` 从 88×36（列 44px）缩至 72×32（列 36px），内部按钮同步 36×32，与导航胶囊同高、同基线（top 一致）。契约断言更新为 72×32。`pnpm check`、257 项测试、`pnpm build` 通过；浏览器实测胶囊与导航按钮同高同基线。
